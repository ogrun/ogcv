...
#' @description Take a position data frame and the section id desired and prints the section to markdown.
#' @param section_id ID of the entries section to be printed as encoded by the `section` column of the `entries` table
section_awsome <- function(cv, section_id){
  ...
  section_data1 <- section_data %>%
    mutate(description_bullets = str_remove_all(description_bullets, "- ")) %>%
    rowwise() %>%
    mutate(description_bullets = strsplit(description_bullets, "\n"))


  section_data_awsome <- tibble(role = section_data1$title,
         company =  section_data1$loc,
         dates = paste(section_data1$start_year, "-" , section_data1$end_year),
         loc = section_data1$institution,
         details = section_data1$description_bullets
         )
  return(section_data_awsome)
}
